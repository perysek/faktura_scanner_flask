...
| Nieobecności (wnioski) | `absence` blueprint | `absence_routes.py` (40/2) | Kompletna (self-service submit/cancel + management-index dociągnięte jako nowe `/api/*`) | Wysoka (3-tabowy widok, conflict-approve modal) | **✅ Zbudowany prawie w całości (Option A rollout 2026-08-24 + deferred-tasks przebieg tego samego dnia) — `/moje-nieobecnosci` (formularz+podgląd konfliktów+historia) i `/nieobecnosci` (taby Wnioski+L4/Manualne+**Kategorie**: zatwierdź/odrzuć/konflikt-modal z force-approve, ręczna rejestracja z pre-check limitu bilansu, usuwanie, CRUD kategorii, **hard-delete superusera** dla nieobecności i kategorii). Świadomie wciąż odłożone: per-konflikt reassign/reschedule (to samo co już odłożone dla Wizyt), historia rozwiązań (zależna od reassign/reschedule), balance-hints w tabeli (kosmetyczne). `npm run build`/`lint` zielone.** |
