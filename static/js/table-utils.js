/**
 * Table Utilities - Enhanced Table Features
...
function sortTable(columnIndex, tableId = 'resultsTable') {
    ...
    while (switching) {
        ...
        for (i = 1; i < (rows.length - 1); i++) {
            ...
            // Try to parse as date first (DD.MM.YYYY or YYYY-MM-DD format)
            const xDate = parseTableDate(xContent);
            const yDate = parseTableDate(yContent);

            if (xDate && yDate) {
                // Both are valid dates - compare as dates
                if (dir == "asc") {
                    if (xDate > yDate) { shouldSwitch = true; break; }
                } else if (dir == "desc") {
                    if (xDate < yDate) { shouldSwitch = true; break; }
                }
            } else {
 ...
 * @param {string} dateStr - Date string from table cell
 * @returns {Date|null} - Parsed date or null if not a valid date
 */
...
function exportToCSV(tableId = 'resultsTable', filename = 'export.csv') {
    ...
    for (let i = 0; i < headers.length; i++) {
        const headerText = headers[i].querySelector('.th-header span')?.textContent || headers[i].textContent;
