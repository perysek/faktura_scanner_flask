...
/**
 * Open the seller-sync modal: check discrepancies then show confirmation list.
 */
async function openSellerSyncModal() {
    const btn = document.getElementById('seller-sync-btn');
    if (btn) btn.disabled = true;

    const loadingModal = Modals.loading('Sprawdzanie powiązań ze sprzedawcami...');

    try {
        const data = await API.invoiceSellerSync.check();
        Modals.close(loadingModal);

        if (!data.success) {
            Notifications.error(data.error || 'Błąd sprawdzania powiązań');
            return;
        }

        const all = [...data.unlinked, ...data.wrong_link];

        if (all.length === 0) {
            Notifications.success('Wszystkie faktury są poprawnie powiązane ze sprzedawcami.');
            return;
        }

        showSellerSyncModal(all);
    } catch (err) {
        Modals.close(loadingModal);
        console.error('Seller sync check error:', err);
        Notifications.error('Błąd sprawdzania powiązań: ' + err.message);
    } finally {
...
/**
 * Build and show the confirmation modal with a table of discrepancies.
 */
function showSellerSyncModal(items) {
    const rows = items.map((item, i) => {
        ...
        return `
            <tr style="border-bottom:1px solid var(--color-border-subtle);">
                ...
                <td style="padding:0.5rem 0.5rem;vertical-align:middle;">${typeLabel}</td>
            </tr>`;
    }).join('');
    ...
    // Store items on window so the apply callback can read them
    window._sellerSyncItems = items;
...
/** Toggle all checkboxes inside the sync modal. */
function sellerSyncToggleAll(checked) {
    document.querySelectorAll('.sync-item-cb').forEach(cb => cb.checked = checked);
}

/** Collect checked items and POST to apply endpoint. */
async function applySellerSyncUpdates() {
    const items = window._sellerSyncItems || [];
    const checked = document.querySelectorAll('.sync-item-cb:checked');
    const indices = new Set([...checked].map(cb => parseInt(cb.dataset.idx)));

    const updates = items
        .filter((_, i) => indices.has(i))
        .map(item => ({ invoice_id: item.invoice_id, seller_id: item.suggested_seller_id }));

    if (updates.length === 0) {
        Notifications.info('Nie zaznaczono żadnych faktur do aktualizacji.');
        return;
    }

    const loadingModal = Modals.loading('Aktualizowanie powiązań...');
    try {
        const result = await API.invoiceSellerSync.apply(updates);
        Modals.close(loadingModal);
        if (result.success) {
            Notifications.success(result.message || `Zaktualizowano ${result.updated_count} faktur`);
            loadInvoices();
        } else {
            Notifications.error(result.error || 'Błąd aktualizacji powiązań');
        }
    } catch (err) {
        Modals.close(loadingModal);
        console.error('Seller sync apply error:', err);
        Notifications.error('Błąd aktualizacji: ' + err.message);
    }
}
