...
**Features**: Post-visit client rating form delivered via SMS + event-triggered SMS on visit
...
P10 (visit started mobile view) — parallel to P07-P09
...
VALUES
    ...
    ('visit_started',
     'Wizyta rozpoczęta',
     'Wysyłana w momencie zmiany statusu wizyty na „w trakcie".',
     FALSE, 0, 0,
     'Hej {client_name}! Twoja wizyta w {salon_name} właśnie się rozpoczęła. '
     'Zapraszamy!',
     FALSE, TRUE, 'in_progress', 11);
```
...
```python
def downgrade():
    op.execute("DELETE FROM sms_message_types WHERE type_key IN ('post_visit_message', 'visit_started')")
...
```python
rating_token:  Optional[str]      = None   # UUID string
...
### 3c. SmsMessageTypeRepository — new method
...
```python
def _trigger_event_sms(appointment_id: int, new_status: str) -> None:
    """Schedule event-triggered SMS for a status transition. Swallows errors."""
    try:
        from repositories.sms.sms_repository import (
            SmsSettingsRepository, SmsMessageTypeRepository,
        )
        ...
        msg_types = SmsMessageTypeRepository().get_event_triggered_by_status(new_status)

        from services.sms_service import SmsService
        svc = SmsService()
        for mt in msg_types:
            delay = mt.get('send_delay_minutes', 0)
            svc.schedule_event_sms(appointment_id, mt['type_key'], delay, base_url)
    except Exception as exc:
        logging.error('_trigger_event_sms failed appt_id=%s status=%s: %s',
                      appointment_id, new_status, exc)
...
if success:
    ...
    # Event-triggered SMS scheduling
    if new_status in (AppointmentStatus.COMPLETED, AppointmentStatus.IN_PROGRESS):
        _trigger_event_sms(appointment_id, new_status)
...
**Note**: `AppointmentStatus.COMPLETED` = `'completed'`, `AppointmentStatus.IN_PROGRESS` = `'in_progress'`.
...
## Phase P10: Visit Started Mobile View (Feature #2)

**New file**: `templates/public/appointment_visit_started.html`

Notification page shown when client opens the `visit_started` SMS link.
Design matches `appointment_confirm.html`.

Reuses `confirmation_token` as the URL token (no new column needed):
`GET /visit-started/<token>` → look up by `confirmation_token`.

### Content

- Header: salon name
- Icon: 💇 or appropriate emoji (rendered as text, not an image)
- Heading: "Twoja wizyta właśnie się rozpoczęła!"
- Details block: date, start time, employee name, services
- Acknowledgment button: "Rozumiem" — POST to mark viewed (optional)
- No rating stars (rating is a separate post-visit flow)

### Route (add to `routes/public_routes.py`)

```python
@public_bp.route('/visit-started/<token>', methods=['GET'])
def visit_started_view(token):
    ...
    client = ClientRepository().get_by_id(appt['client_id'])
    return render_template(
        'public/appointment_visit_started.html',
        appointment=appt,
        client=client,
    )
```

No POST handler needed — this is a notification-only page. The `sms_events` table
already tracks delivery; no additional DB write required on client view.

**SMS template for `visit_started`**: update the seeded template to include the URL:

```
'Hej {client_name}! Twoja wizyta w {salon_name} właśnie się rozpoczęła. '
'Szczegóły: {visit_started_url}'
```

Add `{visit_started_url}` placeholder support to `SmsService._render_template()`:

```python
if '{visit_started_url}' in template_text:
    conf_token = appt.get('confirmation_token', '')
    visit_url = f"{base_url.rstrip('/')}/visit-started/{conf_token}"
    template_text = template_text.replace('{visit_started_url}', visit_url)
```
...
| Risk | Mitigation |
|------|-----------|
| Scheduler fires before `sms_events` row committed | Hook runs synchronously in the route handler before returning response; row is committed before scheduler next wakes |
| Duplicate SMS if scheduler crashes mid-send | Mark event `sent` before sending; on retry, `get_due()` skips already-sent rows |
| `BASE_URL` not set in production | Add config validation on startup; `rate_url` will be broken if missing |
| Rating form accessed before appointment is `completed` | No guard needed — `rating_token` is present but `send()` only fires post-completion; token does not reveal status |
...
- [ ] Changing status to `in_progress` schedules `visit_started` SMS immediately
      (delay = 0 min) and sends on next scheduler tick
- [ ] Client opens `/visit-started/<token>` — sees visit details notification page
...
| **Create** | `templates/public/appointment_visit_started.html` |
